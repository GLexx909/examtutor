...
feature 'User can vote for a comment', %q{
  ...
  given!(:comment) { create :comment, author: user, post: post}

  describe 'Not author vote for a comment' do
    before do
      sign_in(user2)
  ...
  describe 'Author tries to vote for a comment, but can not' do
    before do
      sign_in(user)
      visit post_path(post)
    end
